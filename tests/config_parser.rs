...
mod project_parameters {
    ...
    mod should_succeed {
        ...
        fn when_yaml_is_valid() {
            let test_yml_path = Path::new("tests").join("testdata").join("config").join("config1.yml");
            ...
            assert_eq!(parameters.include_dirs, Some(vec![String::from("include"), String::from("src")]));

            assert_eq!(parameters.lib_paths, Some(vec![String::from("build/libtest.so")]));
        }
    }
    ...
    mod should_fail {
        ...
        fn when_yaml_is_not_existing() {
            let test_yml_path = Path::new("tests").join("testdata").join("config").join("non_existing.yml");
            let parameters_res = ProjectParameters::parse(test_yml_path.to_str().unwrap());
        ...
        fn when_yaml_is_empty() {
            let test_yml_path = Path::new("tests").join("testdata").join("config").join("config3.yml");
            let parameters_res = ProjectParameters::parse(test_yml_path.to_str().unwrap());
        ...
        fn when_no_test_env_could_be_parsed() {
            let test_yml_path = Path::new("tests").join("testdata").join("config").join("config2.yml");
            let parameters_res = ProjectParameters::parse(test_yml_path.to_str().unwrap());
